...
void draw() {
  ...
  for (int counter = 0; counter < asteroidX.size(); counter++) {
    ...
    tint(noise(counter) * 200 + 50, noise(counter+100) * 200 + 50, noise(counter+200) * 200 + 50);
    image(bug, asteroidX.get(counter) - bug.width / 2, asteroidY.get(counter) - bug.height / 2);
  }
  noTint();
